...
test('claiming a hotspot tells the backend, so other dispatchers see it', async () => {
  await withFetch(
    ...
    async (calls) => {
      ...
      assert.equal(calls[0].url, 'http://api.test/api/hotspots/4/claim')
      assert.equal(calls[0].options.method, 'PATCH')
      assert.equal(calls[0].options.headers['Content-Type'], 'application/json')
      assert.deepEqual(JSON.parse(calls[0].options.body), { dispatcherName: 'dispatchera' })
    },
  )
})

test('a hotspot someone else already took comes back as a conflict', async () => {
  await withFetch(
    () => ({ ok: false, status: 409, json: async () => ({ error: 'taken', claimedBy: 'dispatchera' }) }),
    async () => {
...
test('resolving a hotspot sends the token the claim handed back', async () => {
  await withFetch(
    () => ({ ok: true, status: 200, json: async () => ({ resolved: true }) }),
    async (calls) => {
      const r = await resolveHotspot('4', 'tok-1', 'http://api.test')
      assert.equal(r.status, 'resolved')
      assert.equal(calls[0].url, 'http://api.test/api/hotspots/4/resolve')
      assert.equal(calls[0].options.method, 'PATCH')
      assert.deepEqual(JSON.parse(calls[0].options.body), { token: 'tok-1' })
    },
  )
})
